...
const fs = require('fs');      // lo dejamos por si lo usas en otras partes
const path = require('path');  // igual
...
// ===============================
const upload = multer({
  storage: multer.memoryStorage(),       // <--- ya no hay archivos en disco
...
// ============================================
// AUDIO WEATHER ENDPOINT (VOZ) - SIN ARCHIVOS
// ============================================
app.post('/api/audio-weather', upload.single('audio'), async (req, res) => {
  try {
    ...
    // ✅ Usamos el buffer EN MEMORIA
    const { buffer, originalname, mimetype, size } = req.file;
    console.log('📏 Tamaño del audio (bytes):', size);

    // 1. Transcribir el audio usando Whisper de OpenAI
    console.log('📝 Transcribiendo audio desde buffer...');
    const audioFormData = new FormData();
    audioFormData.append('file', buffer, {
